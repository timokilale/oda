...
import { Search, Plus, Sparkles, Filter, Leaf, Info, X } from 'lucide-react';
...
interface GridViewProps {
  ...
  cartQuantities: Record<string, number>; // Maps dishId -> current quantity in cart
}
...
export default function GridView({
  ...
  const categories: (Category | 'All')[] = ['All', 'Starters', 'Mains', 'Desserts', 'Beverages'];
  ...
  const filteredItems = useMemo(() => {
    return items.filter(item => {
      ...
      const matchesVegan = !onlyVegan || item.badges.includes('VEGAN');

      return matchesSearch && matchesCategory && matchesVegan;
    });
  }, [items, searchTerm, selectedCategory, onlyVegan]);

  return (
    <div id="grid-view-panel" className="flex-1 w-full flex flex-col h-full bg-background overflow-hidden">
      ...
      <div className="bg-surface border-b border-border p-4 space-y-3 relative z-10 shadow-sm shrink-0">
        ...
        <div className="relative">
          ...
          <input
            ...
            placeholder="Search Truffle, Hamachi, desserts..."
        ...
        <div className="flex items-center justify-between gap-2 overflow-hidden shrink-0">
          ...
          {/* Vegan filter toggle button */}
          <button
            id="vegan-filter-toggle"
            onClick={() => setOnlyVegan(prev => !prev)}
            className={`min-w-10 h-8 rounded-full border flex items-center justify-center gap-1 text-xs px-2.5 transition-all outline-none ${
              onlyVegan
            ...
            title="Filter Vegan dishes"
          >
            <Leaf className={`w-3.5 h-3.5 ${onlyVegan ? 'fill-success' : ''}`} />
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-28 pt-4">
        {filteredItems.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 text-center text-on-surface-variant/50 space-y-2">
            <Filter className="w-10 h-10 stroke-[1.5]" />
            <h4 className="font-semibold text-sm font-sans text-on-surface">No Culinary Arts Match</h4>
            <p className="text-xs max-w-xs font-sans">
              We couldn't find matches for your search. Try adjusting your filters or browsing other categories.
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 xs:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {filteredItems.map((item) => {
              ...
              return (
                ...
                >
                  {/* Aspect ratio 1:1 top food image */}
                  <div className="relative aspect-square w-full bg-surface-container-low overflow-hidden cursor-pointer" onClick={() => onOpenDetails(item)}>
                    <img
                      src={item.image}
                      alt={item.name}
                    ...
                    <div className="absolute top-2.5 left-2.5 flex flex-wrap gap-1">
                      {item.badges.slice(0, 1).map((b, i) => (
                        <span
                          key={i}
                          className="bg-primary/90 text-white font-sans text-[9px] font-bold tracking-wider px-2 py-0.5 rounded-full backdrop-blur-sm uppercase shadow-sm"
                        >
                          {b}
                        </span>
                      ))}
                  ...
                  <div className="p-3.5 flex-1 flex flex-col justify-between space-y-3">
                    ...
                    {/* Price and actions anchor bottom */}
                    <div className="flex items-center justify-between pt-1.5 border-t border-surface-container">
                      ...
                      {/* Tap to Quick-Add with subtle scale motion */}
                      <button
